...
interface EditSalesReturnProps {
  returnItem: SalesReturn;
  onBack: () => void;
  onSave: (returnItem: SalesReturn) => void;
}

const EditSalesReturn: React.FC<EditSalesReturnProps> = ({
  returnItem,
  onBack,
  onSave,
}) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/sales-returns/${returnItem.id || returnItem._id}`, {
        method: 'PUT',
        headers: {
