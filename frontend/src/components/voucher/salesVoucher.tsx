...
const SalesVoucher: React.FC = () => {
    ...
    setSalesItems((prevItems) =>
      prevItems.map((item) => {
        if (item.id === id) {
          ...
          if (
            ["dzn", "pcs", "rate", "disc", "discPercent", "exDisc"].includes(
              field
            )
          ) {
            ...
            const discPercent =
              field === "discPercent" ? Number(value) : item.discPercent;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(
        "Failed to save sales voucher. Please check if the backend server is running."
      );
    }
  };
  ...
  return (
    <Box
      sx={{
        width: { xs: "100%", md: "90vw" }, // Use 90vw for desktop
        maxWidth: "1400px", // Prevents it from being too wide
        minWidth: { xs: "100%", md: "900px" }, // Ensures a good minimum width
        margin: "0 auto", // Center horizontally
        py: { xs: 2, md: 4 },
        px: { xs: 2, md: 6 },
        minHeight: "100vh",
        backgroundColor: "#FFFFFF",
        overflow: "unset", // Prevents unnecessary scrollbars
      }}
    >
            ...
            {salesItems.map((item, index) => (
              <Card key={item.id} sx={{ mb: 2, p: 2, borderRadius: 2 }}>
                ...
                >
                  ...
                  {salesItems.length > 1 && (
                    <IconButton
                      ...
                      onClick={() => {
                        setSalesItems(
                          salesItems.filter((i) => i.id !== item.id)
                        );
                      }}
                      size="small"
                    >
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  ...
                  <Box
                    sx={{ p: 1, backgroundColor: "#e8f5e8", borderRadius: 1 }}
                  >
                    <Typography
                      variant="body2"
                      color="textSecondary"
